...
-spec be_ok({ok, GVM} | {error, any()}) -> GVM.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, GVR}) -> GVR.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(GVU)) -> GVU.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
