...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FEO)) -> {ok, FEO} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FEI)) -> {ok, FEI} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FEE)) -> {ok, FEE} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FEU), fun((FEU) -> json())) -> json().
...
-spec array(list(FEY), fun((FEY) -> json())) -> json().
