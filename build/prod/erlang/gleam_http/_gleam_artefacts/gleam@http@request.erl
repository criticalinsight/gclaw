...
-spec set_header(request(ELH), binary(), binary()) -> request(ELH).
...
-spec prepend_header(request(ELK), binary(), binary()) -> request(ELK).
...
-spec set_body(request(any()), ELP) -> request(ELP).
set_body(Req, Body) ->
    {request,
...
-spec map(request(ELR), fun((ELR) -> ELT)) -> request(ELT).
...
-spec set_query(request(EMD), list({binary(), binary()})) -> request(EMD).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(EMH), gleam@http:method()) -> request(EMH).
...
-spec set_scheme(request(EMO), gleam@http:scheme()) -> request(EMO).
...
-spec set_host(request(EMR), binary()) -> request(EMR).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(EMU), integer()) -> request(EMU).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(EMX), binary()) -> request(EMX).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(ENA), binary(), binary()) -> request(ENA).
...
-spec remove_cookie(request(ENG), binary()) -> request(ENG).
