...
-spec map_response_body(
    fun((EYX) -> gleam@http@response:response(EYY)),
    fun((EYY) -> EZA)
) -> fun((EYX) -> gleam@http@response:response(EZA)).
...
-spec prepend_response_header(
    fun((EZC) -> gleam@http@response:response(EZD)),
    binary(),
    binary()
) -> fun((EZC) -> gleam@http@response:response(EZD)).
...
-spec ensure_post(gleam@http@request:request(EZG)) -> {ok,
        gleam@http@request:request(EZG)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(EZP)) -> EZR)) -> fun((gleam@http@request:request(EZP)) -> EZR).
