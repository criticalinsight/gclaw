...
-type response(EVB) :: {response, integer(), list({binary(), binary()}), EVB}.
...
-spec set_header(response(EVQ), binary(), binary()) -> response(EVQ).
...
-spec prepend_header(response(EVT), binary(), binary()) -> response(EVT).
...
-spec set_body(response(any()), EVY) -> response(EVY).
...
-spec try_map(response(EVC), fun((EVC) -> {ok, EVE} | {error, EVF})) -> {ok,
        response(EVE)} |
    {error, EVF}.
...
-spec map(response(EWA), fun((EWA) -> EWC)) -> response(EWC).
...
-spec expire_cookie(response(EWL), binary(), gleam@http@cookie:attributes()) -> response(EWL).
