...
-opaque subject(DPF) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DPF)}.

-type name(DPG) :: any() | {gleam_phantom, DPG}.

-type do_not_leak() :: any().

-type selector(DPH) :: any() | {gleam_phantom, DPH}.
...
-spec 'receive'(subject(DQF), integer()) -> {ok, DQF} | {error, nil}.
...
-spec receive_forever(subject(DQN)) -> DQN.
...
-spec selector_receive(selector(DQR), integer()) -> {ok, DQR} | {error, nil}.
...
-spec selector_receive_forever(selector(DQV)) -> DQV.
...
-spec map_selector(selector(DQX), fun((DQX) -> DQZ)) -> selector(DQZ).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRB), selector(DRB)) -> selector(DRB).
merge_selector(A, B) ->
...
-spec select_map(selector(DRM), subject(DRO), fun((DRO) -> DRM)) -> selector(DRM).
...
-spec select(selector(DRI), subject(DRI)) -> selector(DRI).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DRW)
) -> selector(DRW).
...
-spec select_other(selector(DSA), fun((gleam@dynamic:dynamic_()) -> DSA)) -> selector(DSA).
...
-spec deselect(selector(DRR), subject(any())) -> selector(DRR).
...
-spec select_specific_monitor(selector(DSM), monitor(), fun((down()) -> DSM)) -> selector(DSM).
...
-spec select_monitors(selector(DSP), fun((down()) -> DSP)) -> selector(DSP).
...
-spec select_trapped_exits(selector(DRF), fun((exit_message()) -> DRF)) -> selector(DRF).
...
-spec deselect_specific_monitor(selector(DSS), monitor()) -> selector(DSS).
...
-spec send_after(subject(DTO), integer(), DTO) -> timer().
...
-spec perform_call(
    subject(DSV),
    fun((subject(DSX)) -> DSV),
    fun((selector(DSX)) -> {ok, DSX} | {error, nil})
) -> DSX.
...
-spec call(subject(DTC), integer(), fun((subject(DTE)) -> DTC)) -> DTE.
...
-spec call_forever(subject(DTG), fun((subject(DTI)) -> DTG)) -> DTI.
