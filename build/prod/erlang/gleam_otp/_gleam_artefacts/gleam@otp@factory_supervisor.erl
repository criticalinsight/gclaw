...
-opaque supervisor(GCP, GCQ) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(GCP, GCQ))}.

-type message(GCR, GCS) :: any() | {gleam_phantom, GCR, GCS}.

-opaque builder(GCT, GCU) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((GCT) -> {ok, gleam@otp@actor:started(GCU)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(GCT, GCU)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(GCV, GCW) :: {local,
        gleam@erlang@process:name(message(GCV, GCW))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(GCX) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, GCX}.
...
-type erlang_child_spec_property(GCY, GCZ) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((GCY) -> {ok, gleam@otp@actor:started(GCZ)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(GDA, GDB))) -> supervisor(GDA, GDB).
get_by_name(Name) ->
...
-spec named(builder(GDR, GDS), gleam@erlang@process:name(message(GDR, GDS))) -> builder(GDR, GDS).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(GEA, GEB), integer(), integer()) -> builder(GEA, GEB).
...
-spec timeout(builder(GEG, GEH), integer()) -> builder(GEG, GEH).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(GEM, GEN), gleam@otp@supervision:restart()) -> builder(GEM, GEN).
...
-spec start(builder(GES, GET)) -> {ok,
        gleam@otp@actor:started(supervisor(GES, GET))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(GFR, GFS)) -> gleam@otp@supervision:child_specification(supervisor(GFR, GFS)).
supervised(Builder) ->
...
-spec start_child(supervisor(GFY, GFZ), GFY) -> {ok,
        gleam@otp@actor:started(GFZ)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((GGW) -> {ok, gleam@otp@actor:started(GGX)} |
        {error, gleam@otp@actor:start_error()}),
    GGW
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), GGX, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((GDH) -> {ok, gleam@otp@actor:started(GDI)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GDH, GDI).
...
-spec supervisor_child(
    fun((GDM) -> {ok, gleam@otp@actor:started(GDN)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GDM, GDN).
