...
-type message(FGW) :: {message, FGW} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FGX, FGY) :: {continue,
        FGX,
        gleam@option:option(gleam@erlang@process:selector(FGY))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FGZ, FHA) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FGZ,
        gleam@erlang@process:selector(message(FHA)),
        gleam@otp@system:debug_state(),
        fun((FGZ, FHA) -> next(FGZ, FHA))}.

-type started(FHB) :: {started, gleam@erlang@process:pid_(), FHB}.

-opaque initialised(FHC, FHD, FHE) :: {initialised,
        FHC,
        gleam@option:option(gleam@erlang@process:selector(FHD)),
        FHE}.

-opaque builder(FHF, FHG, FHH) :: {builder,
        fun((gleam@erlang@process:subject(FHG)) -> {ok,
                initialised(FHF, FHG, FHH)} |
            {error, binary()}),
        integer(),
        fun((FHF, FHG) -> next(FHF, FHG)),
        gleam@option:option(gleam@erlang@process:name(FHG))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FHI) :: {ack, {ok, FHI} | {error, binary()}} |
...
-spec continue(FHN) -> next(FHN, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FHZ, FIA), gleam@erlang@process:selector(FIA)) -> next(FHZ, FIA).
...
-spec initialised(FIG) -> initialised(FIG, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FIL, any(), FIN),
    gleam@erlang@process:selector(FIR)
) -> initialised(FIL, FIR, FIN).
...
-spec returning(initialised(FIW, FIX, any()), FJC) -> initialised(FIW, FIX, FJC).
...
-spec new(FJG) -> builder(FJG, FJH, gleam@erlang@process:subject(FJH)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FJM)) -> {ok, initialised(FJO, FJM, FJP)} |
        {error, binary()})
) -> builder(FJO, FJM, FJP).
...
-spec on_message(builder(FJY, FJZ, FKA), fun((FJY, FJZ) -> next(FJY, FJZ))) -> builder(FJY, FJZ, FKA).
...
-spec named(builder(FKJ, FKK, FKL), gleam@erlang@process:name(FKK)) -> builder(FKJ, FKK, FKL).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FKY))) -> gleam@erlang@process:selector(message(FKY)).
...
-spec receive_message(self(any(), FKU)) -> message(FKU).
...
-spec initialise_actor(
    builder(any(), any(), FLR),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FLR} | {error, binary()})
...
-spec start(builder(any(), any(), FME)) -> {ok, started(FME)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(FML), FML) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FMN),
    integer(),
    fun((gleam@erlang@process:subject(FMP)) -> FMN)
) -> FMP.
