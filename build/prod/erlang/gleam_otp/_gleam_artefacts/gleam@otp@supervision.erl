...
-type child_specification(GAB) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(GAB)} |
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(GAH)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GAH).
supervisor(Start) ->
...
-spec significant(child_specification(GAM), boolean()) -> child_specification(GAM).
...
-spec timeout(child_specification(GAP), integer()) -> child_specification(GAP).
timeout(Child, Ms) ->
...
-spec restart(child_specification(GAS), restart()) -> child_specification(GAS).
...
-spec map_data(child_specification(GAV), fun((GAV) -> GAX)) -> child_specification(GAX).
